    ...
    jwt.sign(
      ...
      async (err, token) => {
        ...
        const responseUser = {
          ...
          role: user.role,
          userType: user.role,
          name: profile ? (user.role === 'employer' ? profile.companyName : `${profile.firstName} ${profile.lastName}`) : user.email,
...
export const getMe = async (req, res) => {
  try {
    ...
    const responseUser = {
      ...
      name: profile ? (user.role === 'employer' ? profile.companyName : `${profile.firstName} ${profile.lastName}`) : user.email, // Construct name based on role
