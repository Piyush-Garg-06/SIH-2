...
export const getMyPatients = async (req, res) => {
  try {
    ...
    const workers = await Worker.find().select('_id firstName lastName email mobile bloodGroup');
    const patients = await Patient.find().select('_id firstName lastName email mobile bloodGroup');
    
    // Combine workers and patients into a single array
    const allPatients = [
      ...workers.map(worker => ({ ...worker.toObject(), type: 'worker' })),
      ...patients.map(patient => ({ ...patient.toObject(), type: 'patient' }))
    ];
